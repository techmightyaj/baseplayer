...
import com.techmighty.baseplayer.widgets.BubbleTextGetter;
import com.techmighty.baseplayer.widgets.MusicVisualizer;
...
public class SongsListAdapter extends RecyclerView.Adapter<SongsListAdapter.ItemHolder> implements BubbleTextGetter {
    ...
    public void onBindViewHolder(ItemHolder itemHolder, int i) {
        ...
        ImageLoader.getInstance().displayImage(BasePlayerUtils.getAlbumArtUri(localItem.albumId).toString(), itemHolder.albumArt, new DisplayImageOptions.Builder().cacheInMemory(true).showImageOnFail(R.drawable.ic_empty_music2).resetViewBeforeLoading(true).build());
                    ...
                    public boolean onMenuItemClick(MenuItem item) {
                        switch (item.getItemId()) {
                            case R.id.popup_song_play:
                                MusicPlayer.playAll(mContext, songIDs, position, -1, BasePlayerUtils.IdType.NA, false);
                                break;
                            case R.id.popup_song_play_next:
                                long[] ids = new long[1];
                                ids[0] = arraylist.get(position).id;
                                MusicPlayer.playNext(mContext, ids, -1, BasePlayerUtils.IdType.NA);
                                break;
                            ...
                            case R.id.popup_song_addto_queue:
                                ...
                                MusicPlayer.addToQueue(mContext, id, -1, BasePlayerUtils.IdType.NA);
                                break;
    ...
    public class ItemHolder extends RecyclerView.ViewHolder implements View.OnClickListener {
        ...
        public void onClick(View v) {
            ...
            handler.postDelayed(new Runnable() {
                @Override
                public void run() {
                    MusicPlayer.playAll(mContext, songIDs, getAdapterPosition(), -1, BasePlayerUtils.IdType.NA, false);
